...
function CorpusStatistics() {
    ...
    function getBasicDataStructure(nlpResult: Result): DataTableEntry[] {
        ...
        return [
            {
                key: getText("table_key_paragraphs", language),
                value: paragraphCount
            },
            {
                key: getText("table_key_sentences", language),
                value: sentenceCount
            },
            {
                key: getText("table_key_words", language),
                value: wordCount
            },
            {
                key: getText("table_key_sentences_per_paragraph", language),
                value: (sentenceCount / paragraphCount).toFixed(1)
            },
            {
                key: getText("table_key_words_per_paragraph", language),
                value: (wordCount / paragraphCount).toFixed(1)
            },
            {
                key: getText("table_key_words_per_sentence", language),
                value: (wordCount / sentenceCount).toFixed(1)
            },
            {
                key: getText("table_key_average_word_length", language),
                value: averageWordLength.toFixed(1)
            },
            {
                key: getText("table_key_unique_words", language),
                value: nlpResult.words.size
            },
            {
                key: getText("table_key_unique_words_ratio", language),
    ...
    function getDataParagraphs(): DataTableEntry[] {
        ...
        return [
            {
                key: getText("table_key_corpusstat_paragraph_longest", language),
                value: `${longestParagraph} ${getText("table_key_words", language)}`
            },
            {
                key: getText("table_key_corpusstat_paragraph_shortest", language),
                value: `${shortestParagraph} ${getText("table_key_words", language)}`
            },
            {
                key: getText("table_key_corpusstat_paragraph_average", language),
                value: `${averageLength.toFixed(1)} ${getText("table_key_words", language)}`
            }
        ]
    }
    ...
    function getDataSentences(): DataTableEntry[] {
        ...
        return [
            {
                key: getText("table_key_corpusstat_sentence_longest", language),
                value: `${longestSentence} ${getText("table_key_words", language)}`
            },
            {
                key: getText("table_key_corpusstat_sentence_shortest", language),
                value: `${shortestSentece} ${getText("table_key_words", language)}`
            },
            {
                key: getText("table_key_corpusstat_sentence_average", language),
                value: `${averageSentenceLength.toFixed(1)} ${getText("table_key_words", language)}`
            }
        ]
    }
    ...
    function getDataSentenceTypes(): DataTableEntry[] {
        ...
        return [
            {
                key: getText("table_key_corpusstat_sentence_declarative", language),
                value: renderSentendceType(typeDeclarative)
            },
            {
                key: getText("table_key_corpusstat_sentence_question", language),
                value: renderSentendceType(typeQuestion)
            },
            {
                key: getText("table_key_corpusstat_sentence_imperative", language),
                value: renderSentendceType(typeImperative)
            },
            {
                key: getText("table_key_corpusstat_sentence_unknown", language),
                value: renderSentendceType(typeUnknown)
            },
            {
                key: getText("table_key_corpusstat_sentence_speech", language),
    ...
    const columns = [{
        dataField: "key",
        text: getText("label_key", language),
    }, {
        dataField: "value",
        text: getText("label_value", language),
    }];

    return <div className="d-flex flex-column sentence-page justify-content-center" style={{ width: "80%" }}>
        <div className="heading">{getText("label_corpus_statistics_structure", language)}</div>
        <BootstrapTable key="sec1" keyField='key' data={dataStructure} columns={columns} />
        <div className="heading">{getText("label_corpus_statistics_paragraphs", language)}</div>
        <BootstrapTable key="sec2" keyField='key' data={dataParagraphs} columns={columns} />
        <div className="heading">{getText("label_corpus_statistics_sentences", language)}</div>
        <BootstrapTable key="sec3" keyField='key' data={dataSentences} columns={columns} />
        <div className="heading">{getText("label_corpus_statistics_sentencetypes", language)}</div>
